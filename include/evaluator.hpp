...
class Evaluator {
   public:
    /**
     * @brief 引数に与えられた直列化済みの構文ツリーを解析し、結果を返します。
     *
     * @param node 直列化された構文ツリーへのポインタ
     */
...
class FPEvaluator final : public Evaluator<double> {
   private:
    ...
    /**
     * @brief 構文ツリーノードのcontentを読み、数値化して返す
     *
     * @param node 対象のノード
     * @param value 数値化した値の格納先
     * @return bool 変換に失敗した場合はfalseが返ります。
     */
    bool makeValueFromSyntaxNode(const SyntaxNode* node, double& value) const;
